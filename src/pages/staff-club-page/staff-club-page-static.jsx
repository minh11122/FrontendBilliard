...
export const StaffClubPageStatic = () => {
  // loading dùng để hiện trạng thái đang tải khi chưa có dữ liệu thống kê.
  // stats là toàn bộ dữ liệu tổng quan trả về từ API /clubs/staff/statistics.
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState(null);

  // notifications dùng cho popup thông báo của nhân viên.
  // Popup hiện tại đang có logic đầy đủ, nhưng nút chuông đang bị ẩn bằng className="hidden".
  ...
  // Khi đổi tháng hoặc năm, gọi lại API để lấy thống kê đúng khoảng thời gian đang chọn.
  useEffect(() => {
    fetchStats();
  }, [selectedMonth, selectedYear]);

  // Lấy danh sách thông báo một lần khi vào trang tổng quan.
  useEffect(() => {
  ...
  // API thống kê trả về tên CLB, tổng đơn, doanh thu, đánh giá và giải đấu trong tháng/năm đã chọn.
  ...
  // Bộ lọc chỉ cho chọn 12 tháng và 3 năm quanh năm hiện tại.
  const months = Array.from({ length: 12 }, (_, i) => i + 1);
  const years = [currentYear - 1, currentYear, currentYear + 1];

  // Lần đầu vào trang chưa có stats thì hiện dòng tải dữ liệu thay cho dashboard trống.
  if (loading && !stats) {
  ...
  return (
    <div className="p-4 md:p-6 w-full max-w-[1440px] mx-auto min-h-[calc(100vh-80px)] bg-gray-50/50">
      ...
      {/* Header trang và bộ lọc tháng/năm */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          {/* Khối thông báo đã có logic mở popup, nhưng nút chuông đang bị ẩn bằng className="hidden". */}
          ...
          {/* Đổi tháng sẽ cập nhật selectedMonth, sau đó useEffect phía trên tự gọi lại fetchStats. */}
          ...
          {/* Đổi năm sẽ cập nhật selectedYear và tải lại thống kê theo năm được chọn. */}
      ...
      {/* Các thẻ số liệu chính: tổng đơn, doanh thu, đánh giá, giải đấu */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Danh sách nhận xét gần đây trong tháng/năm đang chọn */}
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-5 h-[400px] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
            {stats?.feedbacks?.length === 0 ? (
              ...
              // Mỗi nhận xét hiển thị avatar, tên khách, điểm sao và nội dung bình luận.
        ...
        {/* Danh sách giải đấu trong tháng/năm đang chọn */}
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-5 h-[400px] flex flex-col">
          <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
            <Trophy className="w-5 h-5 text-gray-400" /> Các giải đấu
          </h2>
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
            {stats?.tournaments?.length === 0 ? (
              <div className="text-center text-gray-400 text-sm py-10">Không có giải đấu nào trong tháng</div>
            ) : (
              // Mỗi giải đấu hiển thị tên, số người, ngày bắt đầu, phí tham gia và trạng thái.
              stats?.tournaments?.map(tour => (
                <div key={tour.id} className="p-4 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-[10px] mt-1 uppercase font-bold px-2 py-0.5 rounded-full inline-block ${tour.status === 'Completed' ? 'bg-gray-200 text-gray-600' :
