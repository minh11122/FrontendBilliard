...
export default function OwnerCreateTablePage() {
  ...
  // TỰ ĐỘNG LẤY CLUB ID TỪ TOKEN ĐĂNG NHẬP
  const getClubId = () => {
    const token = localStorage.getItem("token");
    if (!token) return "";
    try {
      const decoded = jwtDecode(token);
      // Bạn có thể console.log(decoded) để xem chính xác tên biến ID là gì
      // Ở đây mình lấy dự phòng các tên biến phổ biến: club_id, shop_id, _id, hoặc id
      return decoded.club_id || decoded.shop_id || decoded._id || decoded.id || "";
    } catch (error) {
