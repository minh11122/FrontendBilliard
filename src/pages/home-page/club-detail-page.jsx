...
export const ClubDetailPage = () => {
  ...
  const fetchClubDetails = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`http://localhost:9999/api/clubs/${id}`);
      if (res.data.success) {
                  ...
                  <div className="flex flex-wrap gap-8 justify-center max-w-lg mt-12 w-full">
                     {availableTables.length > 0 ? availableTables.map(t => (
                        <div 
                          ...
                          className={`w-[140px] h-[80px] rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all relative select-none ${
                        ...
                        >
                           ...
                           <span className="text-[10px] mt-1 text-slate-400 font-bold tracking-wide uppercase">Bàn {selectedTableType}</span>
                        </div>
                     )) : (
