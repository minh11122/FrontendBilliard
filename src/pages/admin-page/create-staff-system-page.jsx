...
export const CreateStaffSystem = () => {
    ...
    <div className="min-h-screen bg-gray-50/60 p-8">
      ...
      <div className="rev-root max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {STAT_CARDS.map(
            ({ label, value, icon: Icon, color, accent, valueClass }) => (
              <div
                key={label}
                className={`stat-card bg-white border ${accent} rounded-2xl p-5 shadow-sm`}
              >
                <div
                ...
                <p
                  className={`text-2xl font-extrabold ${valueClass ?? "text-gray-900"}`}
                >
                  {value}
                </p>
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["Cửa hàng", "Gói dịch vụ", "Giá", "Ngày mua"].map(
                    (h, i) => (
                      <th
                        key={h}
                        className={`px-6 py-3.5 text-xs font-semibold uppercase tracking-wide text-gray-400 ${i === 0 ? "text-left" : i === 3 ? "text-right" : "text-left"}`}
                      >
                        {h}
                      </th>
                    ),
              ...
              <tbody className="divide-y divide-gray-50">
                {transactions.map((item) => (
                  <tr key={item._id} className="rev-row hover:bg-gray-50/80">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-full ${getPkgColor(item.subscription_name)}`}
                      >
