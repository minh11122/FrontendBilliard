...
export default function OwnerCreateTournamentPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Có lỗi xảy ra khi tạo giải đấu");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex-1 p-6 lg:p-10 max-w-4xl mx-auto w-full">
      ...
      <div className="hidden lg:flex items-center gap-2 text-sm mb-6">
        ...
        <span className="text-slate-900 font-medium">Tạo giải mới</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <Trophy className="w-7 h-7 text-amber-500" /> Tạo Giải đấu mới
            </h1>
            <p className="text-slate-500 text-sm mt-0.5">Điền thông tin để tổ chức giải đấu billiard.</p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button type="button" onClick={handleSubmit} disabled={submitting || Object.keys(errors).length > 0}
            ...
            <Save className="w-5 h-5" /> {submitting ? "Đang lưu..." : "Lưu giải đấu"}
          </button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 lg:p-8">
          ...
          <p className="text-xs text-slate-400 mb-5">Ảnh banner sẽ hiển thị ở trang chủ giải đấu, giúp thu hút sự chú ý của người chơi.</p>
